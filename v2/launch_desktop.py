...
# v2: port from env var, default 5003 (v1 runs on 5002).
...
def run_server():
    ...
    app_module.app.run(debug=False, port=_PORT, host='127.0.0.1', threaded=True, use_reloader=False)

def wait_for_server(url=None, timeout=15, interval=0.3):
    if url is None:
...
def _macos_activate_app():
    ...
    try:
        ...
        try:
            ...
            logging.debug("silent except at launch_desktop.py:59", exc_info=True)
