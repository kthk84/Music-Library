        ...
        for line in out.stdout.splitlines():
            if "estimated duration:" in line:
                try:
                    ...
                    logging.debug("silent except at scan_music_folders.py:96", exc_info=True)
                    ...
                    for p in parts:
                        ...
                        if p.endswith("ch") and info["channels"] is None:
                            try:
                                ...
                                logging.debug("silent except at scan_music_folders.py:107", exc_info=True)
                        if "Hz" in p and info["sample_rate"] is None:
                            try:
                                info["sample_rate"] = int(p.replace("Hz", "").strip())
                            except ValueError:
                                logging.debug("silent except at scan_music_folders.py:112", exc_info=True)
        return info
