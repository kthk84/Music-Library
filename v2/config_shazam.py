...
def _migrate_user_to_project_config() -> None:
    ...
    if os.path.exists(user) and not os.path.exists(proj):
        try:
            ...
            logging.debug("silent except at config_shazam.py:70", exc_info=True)
...
def _load_config_with_restore(config: dict) -> dict:
    ...
    try:
        ...
        logging.debug("silent except at config_shazam.py:111", exc_info=True)
    return config
...
def _resolve_profile_display_name(user_data_dir: str, name: str) -> str:
    ...
    try:
        ...
        logging.debug("silent except at config_shazam.py:169", exc_info=True)
    return name
