...
# Helpers extracted to lib/ in session 9 (2026-04-29). The names below remain
...
def _get_search_favorite_logger():
    ...
    try:
        ...
        logging.debug("silent except at app.py:56", exc_info=True)
    return _log
...
def _get_soundeo_download_logger():
    ...
    try:
        ...
        logging.debug("silent except at app.py:77", exc_info=True)
    return _log
    ...
    try:
        ...
        if audio.get('COMM'):
            try:
                ...
                logging.debug("silent except at app.py:160", exc_info=True)
            ...
            if data.get('results') and data['results'].get('trackmatches'):
                ...
                for track in tracks[:5]:
                    ...
                    try:
                        ...
                        logging.debug("silent except at app.py:386", exc_info=True)
...
def _set_url_variants(status: Dict, key: str, url: str) -> None:
    ...
    try:
        ...
        logging.debug("silent except at app.py:553", exc_info=True)
    try:
        ...
        logging.debug("silent except at app.py:564", exc_info=True)
    for v in variants:
        if v:
            status['urls'][v] = url
...
def rank_result(result: Dict, existing_title: str = '', existing_artist: str = '', filename: str = '', all_results: list = None) -> float:
    ...
    if all_results and result.get('year'):
        try:
            ...
            for r in all_results:
                if r.get('year'):
                    try:
                        ...
                        logging.debug("silent except at app.py:796", exc_info=True)
            ...
            logging.debug("silent except at app.py:811", exc_info=True)
...
def _fetch_shazam_from_db():
    ...
    try:
        ...
        logging.debug("silent except at app.py:1104", exc_info=True)
    ...
    if not new_tracks:
        ...
        if db_path and os.path.exists(db_path):
            try:
                ...
                logging.debug("silent except at app.py:1113", exc_info=True)
    return new_tracks
...
def _run_compare_background():
    ...
    try:
        ...
        if not configured_folders:
            ...
            to_dl = [
                {'artist': t['artist'], 'title': t['title'], **({'shazamed_at': t['shazamed_at']} if t.get('shazamed_at') is not None else {}), **({'shazamed_count': t['shazamed_count']} if t.get('shazamed_count') is not None else {})}
            ...
            for idx, s in enumerate(sorted_tracks):
                ...
                if s.get('shazamed_count') is not None:
                    item['shazamed_count'] = s['shazamed_count']
        ...
        try:
            ...
            logging.debug("silent except at app.py:1358", exc_info=True)
...
def shazam_sync_compare():
    ...
    if not shazam_tracks:
        try:
            ...
            logging.debug("silent except at app.py:1386", exc_info=True)
    if not shazam_tracks:
        return jsonify({
    ...
    if cache_valid:
        ...
        to_download = [t for t in to_download_raw if (t['artist'].strip().lower(), t['title'].strip().lower()) not in skipped]
        to_download = _dedupe_tracks_by_key(to_download)
        skipped_tracks = [{'artist': t['artist'], 'title': t['title'], **({'shazamed_at': t['shazamed_at']} if t.get('shazamed_at') is not None else {}), **({'shazamed_count': t['shazamed_count']} if t.get('shazamed_count') is not None else {})}
...
def _rebuild_status_from_caches():
    ...
    if not folder_paths:
        skipped = load_skip_list()
        to_dl = [{'artist': t['artist'], 'title': t['title'], **({'shazamed_at': t['shazamed_at']} if t.get('shazamed_at') is not None else {}), **({'shazamed_count': t['shazamed_count']} if t.get('shazamed_count') is not None else {})}
    ...
    if not local_scan_cache_valid(local_scan, folder_paths):
        ...
        if old_status and (old_status.get('to_download') or old_status.get('have_locally')):
            ...
            for t in old_status.get('skipped_tracks', []):
                existing_keys.add((_track_key_norm(t)))
            new_tracks = [
                {**({'shazamed_at': t['shazamed_at']} if t.get('shazamed_at') is not None else {}), **({'shazamed_count': t['shazamed_count']} if t.get('shazamed_count') is not None else {}),
...
def _merge_preserved_urls_into_status(status: Dict) -> None:
    ...
    # Self-heal cover_hashes from disk if sparse vs urls. cover_cache files are
    # named <md5(key)>.jpg, so the mapping is deterministically reconstructable.
    # Cheap (md5 of url keys), idempotent (only fills missing entries).
    try:
        cover_hashes = status.get('cover_hashes') or {}
        urls = status.get('urls') or {}
        # Run the disk rebuild whenever the persisted file is missing covers vs
        # the keys we have URLs for (with a small buffer for variant aliasing).
        if urls and len(cover_hashes) < max(1, int(len(urls) * 0.8)):
            added = _rebuild_cover_hashes_from_disk(status)
            if added:
                logging.info("cover_hashes self-healed from disk: +%d entries", added)
    except Exception:
        logging.exception("cover_hashes self-heal failed")
    # Self-heal #2: extract embedded artwork from local files for have_locally
    # tracks that still lack a cover_hash. This recovers the ~98% of downloaded
    # AIFFs that have APIC frames but were never linked into cover_hashes.
    try:
        cover_hashes = status.get('cover_hashes') or {}
        have_local = status.get('have_locally') or []
        # Count how many local tracks lack a hash — only run extraction if
        ...
        for t in have_local:
            ...
            if a and ti:
                base = f'{a} - {ti}'
                if base not in cover_hashes and base.lower() not in cover_hashes:
                    missing_local += 1
                    if missing_local > 20:
                        break
        if missing_local > 20:
            added2 = _extract_local_artwork_to_cache(status)
            if added2:
                logging.info("Embedded APIC artwork extracted: +%d covers from local files", added2)
    except Exception:
        logging.exception("APIC artwork self-heal failed")
    # Self-heal #3: auto-trigger Soundeo backfill when a SMALL number of url-keys
    # lack covers (e.g. tracks Shazammed since the last bulk backfill that didn't
    # get cover cached at search-time). Throttled and bounded so we never spam
    # Soundeo or run during a bulk-load.
    try:
        _auto_trigger_cover_backfill_if_small_gap(status)
    except Exception:
        logging.exception("auto-backfill trigger failed")


# --- Auto-backfill: bridge between in-memory self-heal and the Soundeo HTTP
# fetch worker. Runs from inside _merge_preserved_urls_into_status when there's
# a small post-Shazam gap. Throttled so a stuck error condition doesn't loop.
...
def _auto_trigger_cover_backfill_if_small_gap(status: Dict) -> None:
    """If 1..MAX url-keys lack a cover_hash, fire the existing background backfill
    worker. Variant-aware (key vs key.lower) so the membership check matches what
    the renderer actually looks up. Throttled to one trigger per 10 min."""
    global _AUTO_BACKFILL_LAST_AT
    global _cover_backfill_running, _cover_backfill_progress
    if not status:
        return
    # Stuck-state recovery — handles two failure modes seen empirically:
    #
    # (A) Bool stuck, dict reset:  _cover_backfill_running=True but
    #     _cover_backfill_progress['running']=False. Means the worker's `finally`
    #     reset the dict but not the bool (likely thread cleanup edge case).
    #     Fix: trust the dict, reset the bool.
    #
    # (B) Both stuck (worker died mid-run): _cover_backfill_running=True AND
    #     _cover_backfill_progress['running']=True with started_at far in the past.
    #     Means the worker thread terminated abnormally without finally completing.
    #     Fix: 30-min watchdog (full library backfill of 3000+ covers takes ~17 min,
    #     so anything past 30 min is definitively stuck).
    p = _cover_backfill_progress or {}
    dict_running = bool(p.get('running', False))
    started_at = p.get('started_at') or 0
    elapsed = (time.time() - started_at) if started_at else 0

    # (A) Single-side stuck
    if _cover_backfill_running and not dict_running:
        logging.warning("auto-backfill: stuck _cover_backfill_running=True with progress.running=False; resetting")
        _cover_backfill_running = False
    # (B) Watchdog — both look running but started_at is suspiciously old
    elif (_cover_backfill_running or dict_running) and started_at and elapsed > 1800:
        logging.warning("auto-backfill: stale backfill state (elapsed=%ds); force-resetting", int(elapsed))
        _cover_backfill_running = False
        _cover_backfill_progress['running'] = False

    if _cover_backfill_running:
        return
    if (time.time() - _AUTO_BACKFILL_LAST_AT) < _AUTO_BACKFILL_MIN_INTERVAL_SEC:
        return
    urls = status.get('urls') or {}
    if not urls:
        return
...
def _add_starred_lowercase_aliases(status: Dict) -> Dict:
    ...
    out = dict(status)
    for m in ('urls', 'starred', 'soundeo_titles', 'dismissed', 'not_found', 'cover_hashes', 'maybe', 'listened'):
...
def _reconcile_to_download_with_local_cache(status: Dict) -> Dict:
    ...
    for t in to_dl:
        ...
        if known_fp and os.path.exists(known_fp):
            ...
            if t.get('shazamed_count') is not None:
                item['shazamed_count'] = t['shazamed_count']
            have.append(item)
...
def shazam_sync_status():
    ...
    # Soundeo. Hooked into the route directly because _merge_preserved_urls_into_status
    # only runs on rebuild paths, but the typical UI poll uses the cached fast path.
    try:
        _auto_trigger_cover_backfill_if_small_gap(out)
    except Exception:
        logging.exception("auto-backfill route hook failed")
    if compare_running:
    ...
    with _proxy_lock:
        ...
        if entry['refcount'] <= 0:
            ...
            try:
                ...
                logging.debug("silent except at app.py:2160", exc_info=True)
    return jsonify({'ok': True})
        ...
        with _proxy_lock:
            ...
            for pid in to_remove:
                ...
                try:
                    ...
                    logging.debug("silent except at app.py:2181", exc_info=True)
    ...
    try:
        if sys.platform == 'darwin':
            ...
            try:
                ...
                logging.debug("silent except at app.py:2313", exc_info=True)
...
def _run_cover_backfill():
    ...
    try:
        ...
        try:
            ...
            logging.debug("silent except at app.py:2424", exc_info=True)
        if not cookies_path:
        ...
        _cover_backfill_progress = {'done': 0, 'total': len(todo), 'running': True, 'started_at': time.time()}
        ...
        for i, (key, track_url) in enumerate(todo):
            ...
            if (i + 1) % 25 == 0:
                try:
                    ...
                    logging.debug("silent except at app.py:2501", exc_info=True)
            time.sleep(0.1)
        ...
        try:
            ...
            logging.debug("silent except at app.py:2508", exc_info=True)
...
def _run_rescan_only_background():
    ...
    try:
        ...
        try:
            ...
            logging.debug("silent except at app.py:2741", exc_info=True)
...
def shazam_sync_rescan():
    ...
    try:
        ...
        logging.debug("silent except at app.py:2754", exc_info=True)
    ...
    def _run():
        try:
            ...
            try:
                ...
                logging.debug("silent except at app.py:2875", exc_info=True)
...
def _run_download_queue_worker():
    ...
    for i, key in enumerate(keys):
        ...
        save_status_cache(status)
    # --- 404 auto-retry pass ------------------------------------------------
    # When Soundeo returns 404 on /download/{id}/{format}, the most common cause
    # is that they removed the track from their catalog after this app matched
    # it — the track page still resolves, but the file is gone. The track may
    # still exist under a slightly different listing (different remix, artist
    # permutation, relisted ID), so before declaring defeat we re-Search via
    # HTTP (no Selenium → no driver contention) and retry once with any new URL.
    # Tracks that still 404 after this pass are genuinely gone.
    if not no_credits and cookies_path and results:
        retry_keys_404 = []
        original_urls = {}
        for r in results:
            err_str = str(r.get('error', '') or '')
            if not r.get('ok') and '404' in err_str and 'Soundeo' in err_str:
                k = r.get('key')
                if k and k not in retry_keys_404:
                    retry_keys_404.append(k)
                    original_urls[k] = (urls.get(k) or '').strip()
        if retry_keys_404:
            try:
                from soundeo_automation import run_search_tracks_http
                retry_tracks = []
                for k in retry_keys_404:
                    if ' - ' in k:
                        a, t = k.split(' - ', 1)
                        a, t = a.strip(), t.strip()
                        if a or t:
                            retry_tracks.append({'artist': a, 'title': t})
                if retry_tracks:
                    dlog.info("download_queue: 404 auto-retry pass: re-searching %d track(s)", len(retry_tracks))
                    app._shazam_download_progress['message'] = (
                        f"Re-searching {len(retry_tracks)} 404'd track(s) on Soundeo…"
                    )
                    search_out = run_search_tracks_http(retry_tracks, cookies_path) or {}
                    new_urls = search_out.get('urls') or {}
                    new_titles = search_out.get('soundeo_titles') or {}
                    # Persist any new URLs into status so future queue runs use them.
                    status_now = dict(load_status_cache() or getattr(app, '_shazam_sync_status', None) or {})
                    urls_map = status_now.setdefault('urls', {})
                    titles_map = status_now.setdefault('soundeo_titles', {})
                    persisted_new = 0
                    for k, u in new_urls.items():
                        if u and u != urls_map.get(k):
                            urls_map[k] = u
                            persisted_new += 1
                        if new_titles.get(k):
                            titles_map[k] = new_titles[k]
                    if persisted_new:
                        save_status_cache(status_now)
                        app._shazam_sync_status = status_now
                        dlog.info("download_queue: 404 retry: %d new URL(s) persisted to status", persisted_new)
                    # Try one HTTP download per track that got a *different* URL.
                    recovered = 0
                    for k in retry_keys_404:
                        new_url = (new_urls.get(k) or '').strip()
                        if not new_url:
                            dlog.info("download_queue: 404 retry skip key=%s — no new URL from search", k[:60])
                            continue
                        if new_url == original_urls.get(k):
                            dlog.info("download_queue: 404 retry skip key=%s — search returned same URL (truly gone)", k[:60])
                            continue
                        dlog.info("download_queue: 404 retry: key=%s new_url=%s", k[:60], new_url[:80])
                        app._shazam_download_progress['message'] = f"Retrying after re-search: {k[:50]}..."
                        out = soundeo_download_aiff(new_url, cookies_path, dest_dir, k)
                        # Replace the matching failed entry in `results`.
                        for ri, r in enumerate(results):
                            if r.get('key') == k and not r.get('ok'):
                                if out.get('ok'):
                                    results[ri] = {
                                        'key': k,
                                        'ok': True,
                                    ...
                                    recovered += 1
                                    try:
                                        _apply_download_to_status_and_cache(k, out.get('filepath', ''))
                                    except Exception as e:
                                        dlog.warning("download_queue: retry apply status failed: %s", e)
                                    dlog.info("download_queue: 404 retry RECOVERED key=%s", k[:60])
                                else:
                                    new_err = out.get('error', 'unknown')
                                    results[ri] = {
                                        'key': k,
                                        'ok': False,
                                        'error': new_err,
                                        'retried_after_404': True,
                                    }
                                    dlog.info("download_queue: 404 retry STILL FAILED key=%s error=%s", k[:60], new_err)
                                break
                    app._shazam_download_progress['done'] = done
                    app._shazam_download_progress['failed'] = failed
                    app._shazam_download_progress['results'] = results
                    if recovered:
                        dlog.info("download_queue: 404 auto-retry pass recovered %d track(s)", recovered)
            except Exception as e:
                dlog.warning("download_queue: 404 auto-retry pass failed: %s", e)
    # --- /404 auto-retry pass -----------------------------------------------
...
def shazam_sync_download_track():
    ...
    if track_url and track_url.startswith('http'):
        try:
            ...
            logging.debug("silent except at app.py:3246", exc_info=True)
...
def shazam_sync_favorites_from_soundeo():
    ...
    if request.get_data():
        try:
            ...
            logging.debug("silent except at app.py:4112", exc_info=True)
...
def shazam_search_soundeo_global():
    ...
    if request.get_data():
        try:
            ...
            logging.debug("silent except at app.py:4189", exc_info=True)
...
def shazam_sync_run_soundeo():
    ...
    if request.get_data():
        try:
            ...
            logging.debug("silent except at app.py:4229", exc_info=True)
    if not tracks:
...
def _unstar_one_track_impl(key: str, track_url: str):
    ...
    if track_url and not (soundeo_result and soundeo_result.get('ok')):
        ...
        try:
            ...
            logging.debug("silent except at app.py:4423", exc_info=True)
        finally:
            _graceful_quit(driver)
    soundeo_ok = bool(soundeo_result and soundeo_result.get('ok'))
    if soundeo_result and soundeo_result.get('ok') and track_url and cookies_path:
...
def _set_url_and_track_id(status: Dict, key: str, url: str, cookies_path: Optional[str] = None) -> None:
    ...
    if not had_url:
        try:
            ...
            logging.debug("silent except at app.py:4643", exc_info=True)
    if not cookies_path:
        return
    key_lower = key.lower() if isinstance(key, str) else ''
...
def shazam_sync_undismiss_track():
    ...
    if not soundeo_ok and artist and title:
        ...
        try:
            ...
            logging.debug("silent except at app.py:4769", exc_info=True)
        finally:
            _graceful_quit(driver)
...
def _star_one_track_impl(key: str, track_url: str, artist: str, title: str) -> Tuple[bool, str]:
    ...
    if not soundeo_ok and artist and title:
        ...
        try:
            ...
            logging.debug("silent except at app.py:4871", exc_info=True)
        finally:
            _graceful_quit(driver)
...
def soundeo_browser_check():
    ...
    try:
        ...
        logging.debug("silent except at app.py:5583", exc_info=True)
    out["ok"] = True
...
def soundeo_session_saved():
    ...
    if os.path.exists(cookies_path):
        try:
            ...
            logging.debug("silent except at app.py:5618", exc_info=True)
...
def scan_folder():
    ...
    try:
        ...
        logging.debug("silent except at app.py:5658", exc_info=True)
    ...
    for update in updates:
        ...
        try:
            ...
            try:
                ...
                logging.debug("silent except at app.py:6041", exc_info=True)
    ...
    try:
        ...
        if not os.path.exists(file_path):
            ...
            try:
                ...
                logging.debug("silent except at app.py:6134", exc_info=True)
...
def _eager_load_shazam_status():
    ...
    try:
        ...
        logging.debug("silent except at app.py:6163", exc_info=True)


_eager_load_shazam_status()


# --- Personal curation flags (maybe / listened) — local-only ---------------
# These flags live alongside `urls`/`starred`/etc. in shazam_status_cache.json
# but NEVER sync to Soundeo. They give Keith a 3-state interest model
# (skip / maybe / star) plus a separate "listened" dimension for the
# scan-the-list workflow.

def _set_local_flag_variants(status: Dict, dict_name: str, key: str, value: bool) -> None:
    """Set or clear a `maybe` / `listened` flag under all variants of `key`.

    Mirrors the variant-write pattern of `_set_url_variants` so the client's
    multi-variant lookup (`_lu(dict, key, keyLower, keyNorm, keyNormLower,
    keyDeep)`) always hits no matter what form the row's data-track-key takes.
    Removing (`value=False`) clears every variant so the flag stays consistent.
    """
    if not status or not key:
        return
    flag_dict = status.setdefault(dict_name, {})
    k = (key or '').strip()
    if not k:
        return
    variants = {k, k.lower()}
    try:
        ...
        logging.debug("_set_local_flag_variants strip-parens failed", exc_info=True)
    try:
        deep = _deep_norm_key(k)
        if deep:
            variants.add(deep)
        ...
        logging.debug("_set_local_flag_variants deep-norm failed", exc_info=True)
    if value:
        for v in variants:
            if v:
                flag_dict[v] = True
        ...
        for v in variants:
            flag_dict.pop(v, None)


@app.route('/api/shazam-sync/mark-maybe', methods=['POST'])
def shazam_mark_maybe():
    """Toggle the 'maybe' flag (local-only personal curation; never touches Soundeo)."""
    body = request.get_json(silent=True) or {}
    ...
    from shazam_cache import load_status_cache, save_status_cache
    status = load_status_cache() or {}
    _set_local_flag_variants(status, 'maybe', key, value)
    mem = getattr(app, '_shazam_sync_status', None)
    if mem is not None:
        _set_local_flag_variants(mem, 'maybe', key, value)
    try:
        save_status_cache(status)
    except Exception as e:
...
@app.route('/api/shazam-sync/mark-listened', methods=['POST'])
def shazam_mark_listened():
    """Toggle the 'listened' flag (local-only; never touches Soundeo)."""
    body = request.get_json(silent=True) or {}
    key = (body.get('key') or '').strip()
    if not key:
        return jsonify({'error': 'key required'}), 400
    ...
    from shazam_cache import load_status_cache, save_status_cache
    status = load_status_cache() or {}
    _set_local_flag_variants(status, 'listened', key, value)
    mem = getattr(app, '_shazam_sync_status', None)
    if mem is not None:
        _set_local_flag_variants(mem, 'listened', key, value)
    try:
        save_status_cache(status)
    except Exception as e:
        return jsonify({'error': f'save failed: {e}'}), 500
    return jsonify({'ok': True, 'key': key, 'value': value})
# --- /Personal curation flags ----------------------------------------------


# --- Version toggle (v1 ↔ v2) -----------------------------------------------
# Two modes:
#   Supervised  — launched via SoundBridge.command. Switch writes a marker file
#                 and exits; the supervisor restarts the OTHER version on the
#                 SAME port (stable URL across switches).
#   Standalone  — launched directly via `python3 app.py`. Switch spawns the
#                 OTHER version as a detached subprocess on its default port,
#                 returns the new URL for the browser to redirect to, then
#                 self-exits so only one version is alive at a time.
# Both modes share state files (cover_cache, browser profile, status cache).

_SOUNDBRIDGE_SWITCH_FILE = '/tmp/soundbridge-switch'

_VERSION_PORTS = {'v1': 5002, 'v2': 5003}
_VERSION_FOLDERS = {'v1': 'SoundBridge', 'v2': 'SoundBridge v2'}
_THIS_VERSION = 'v2'


def _is_port_alive(port: int) -> bool:
    """Quick TCP check: return True if something is listening on 127.0.0.1:port."""
    import socket
    ...
    try:
        ...
        try:
            ...
            logging.debug("silent except: socket close in _is_port_alive", exc_info=True)


def _resolve_other_version_dir(target: str) -> Optional[str]:
    """Find the sibling folder for the target version, relative to this file."""
    here = os.path.dirname(os.path.abspath(__file__))
    parent = os.path.dirname(here)
    folder = _VERSION_FOLDERS.get(target)
    if not folder:
        return None
...
def _spawn_other_version(target: str) -> Optional[int]:
    """Spawn the other SoundBridge version as a detached subprocess.
    Returns the port it should listen on, or None on failure."""
    import subprocess
    target_dir = _resolve_other_version_dir(target)
    if not target_dir:
        return None
    target_port = _VERSION_PORTS[target]
    here = os.path.dirname(os.path.abspath(__file__))
    parent = os.path.dirname(here)
    env = os.environ.copy()
    # Always point at v1's data dir so v1 and v2 share state
    env['SOUNDBRIDGE_DATA_DIR'] = os.path.join(parent, 'SoundBridge')
    env['SOUNDBRIDGE_PORT'] = str(target_port)
    env.pop('SOUNDBRIDGE_SUPERVISOR', None)  # spawned child is NOT supervised
    try:
        subprocess.Popen(
            ['python3', 'app.py'],
            cwd=target_dir,
        ...
        logging.warning("_spawn_other_version: failed to spawn %s: %s", target, e)
        return None
...
def version_info():
    return jsonify({
        ...
        'supervised': bool(os.environ.get('SOUNDBRIDGE_SUPERVISOR')),
        'port': int(os.environ.get('SOUNDBRIDGE_PORT', str(_VERSION_PORTS[_THIS_VERSION]))),
    })


@app.route('/api/version/switch', methods=['POST'])
def version_switch():
    target = ((request.get_json(silent=True) or {}).get('to') or '').strip()
    if target not in ('v1', 'v2'):
        return jsonify({'error': "invalid 'to' (must be 'v1' or 'v2')"}), 400
    if target == _THIS_VERSION:
        return jsonify({'error': f'already running {_THIS_VERSION}'}), 400

    # Mode 1: SUPERVISED — write marker + exit; supervisor restarts on same port.
    if os.environ.get('SOUNDBRIDGE_SUPERVISOR'):
        try:
            with open(_SOUNDBRIDGE_SWITCH_FILE, 'w') as f:
                f.write(target)
        except OSError as e:
            return jsonify({'error': f'could not write switch file: {e}'}), 500
        threading.Thread(
            target=lambda: (__import__('time').sleep(0.5), os._exit(0)),
            daemon=True,
        ).start()
        return jsonify({'switching_to': target, 'mode': 'supervisor'})

    # Mode 2: STANDALONE — spawn the other version, return its URL, exit self.
    target_port = _VERSION_PORTS[target]
    if not _is_port_alive(target_port):
        spawned = _spawn_other_version(target)
        if not spawned:
            return jsonify({
                'error': f'could not spawn {target}',
                'hint': 'Try launching via SoundBridge.command in the parent folder for stable switching.',
            }), 500
        # Wait up to ~10s for it to come up
        import time as _time
        for _ in range(40):
            if _is_port_alive(target_port):
                break
            _time.sleep(0.25)
        if not _is_port_alive(target_port):
            return jsonify({'error': f'spawned {target} but it did not start in time'}), 500

    # Self-exit shortly so only one version is alive at a time.
    threading.Thread(
        target=lambda: (__import__('time').sleep(3), os._exit(0)),
        daemon=True,
    ).start()
    return jsonify({
        'switching_to': target,
        'mode': 'standalone',
        'redirect_url': f'http://127.0.0.1:{target_port}/',
    })
# --- /Version toggle --------------------------------------------------------
